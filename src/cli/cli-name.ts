...
const DEFAULT_CLI_NAME = "visual-studio";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(visual-studio)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `visual-studio`. */
...
/** Replace a leading `visual-studio` command prefix with the active CLI name. */
