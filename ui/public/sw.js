// Visual Studio – Service Worker
// Handles offline caching and push notifications.

const CACHE_PREFIX = "visualstudio-control-";
const EMBEDDED_CACHE_VERSION = "__VS_CONTROL_UI_BUILD_ID__";
const URL_CACHE_VERSION = new URL(self.location.href).searchParams
  .get("v")
  ?.replace(/[^a-zA-Z0-9._-]/g, "-");
const CACHE_VERSION =
  (EMBEDDED_CACHE_VERSION !== "__VS_CONTROL_UI_BUILD_ID__"
...
self.addEventListener("push", (event) => {
  ...
  const options = {
    ...
    tag: data.tag || "visualstudio-notification",
    data: { url: data.url || "./" },
  };
